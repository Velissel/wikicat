...
import { chain, get, isEmpty, isNil } from 'lodash';
import { ServerResponse } from '../../common/response.middleware';
import api from '../../utils/api';
import { ServerError } from '../../common/error.middleware';

export const BREED_ID_MISSING: ServerError = new Error(
  'breed id is not supplied',
);
...
export default async function breedByIdController(
  ...
  const images = await api.get<unknown[]>(`/images/search`, {
    params: {
      breed_ids: id,
      limit: 10,
    },
  ...
  res.body = {
    breed: data,
    images: chain(images).get('data', []).map('url').value(),
  };
  next();
}
